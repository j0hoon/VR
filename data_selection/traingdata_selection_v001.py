...
def main():
    ...
    balanced_result = total_result.query("FVL < 1800 and FVI < 1800 and FVR < 1800 and RVL < 1800 and RVR < 1800 and RVI > 0 and AVL > 0 and AVR >0")
    ...
    tmp_b = balanced_result.loc[:,["FVL","FVI","FVR","AVL","AVR","RVL","RVI","RVR"]]
    tmp_3 = tmp_b.mean()    
    
    data_b = {'Recognition':["FVL","FVI","FVR","AVL","AVR","RVL","RVI","RVR"],
            'frame':[tmp_3['FVL'],tmp_3['FVI'],tmp_3['FVR'],tmp_3['AVL'],tmp_3['AVR'],tmp_3['RVL'],tmp_3['RVI'],tmp_3['RVR']]}
    
    result_df_b = pd.DataFrame(data_b)
    sns.barplot(x='Recognition',y='frame',data = result_df_b)
    plt.grid(True)
    plt.show()   
